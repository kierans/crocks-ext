...
describe("JSON", function() {
	describe("parsing", function() {
		it("should parse JSON", function() {
			const result = parse('{ "a": 1 }').either(throwError, identity)

			assertThat(result, is(equalTo({ a: 1 })))
		});

		it("should return error on parsing error", function() {
			const result = parse('{').either(identity, throwError)
	...
	describe("stringifying", function() {
		it("should stringify data", function() {
			...
			const result = stringify(data).either(throwError, identity)
		...
		it("should return error on stringify error", function() {
			...
			const result = stringify(data).either(identity, throwError)
		...
		it("should pretty print json", function() {
			...
			const result = pretty(2)(data).either(throwError, identity)
